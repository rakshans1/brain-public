...
class ObsidianSync {
    ...
    async shouldPublish(filePath) {
        try {
            ...
            console.error(chalk.red(`Error checking publish status for ${filePath}:`), error);
            return false;
        }
    }
    ...
    async copyAssets(sourceFilePath, destFilePath) {
        try {
            ...
            console.error(chalk.yellow(`Warning: Error copying assets for ${sourceFilePath}:`), error);
        }
    }
    ...
    async processFile(file) {
        try {
            ...
            const destPath = path.join(this.config.brainPublicContent, relativePath);
    ...
    async sync() {
        ...
        try {
            const items = await fs.readdir(this.config.brainPublicContent, { withFileTypes: true });
            for (const item of items) {
                if (item.name !== '.git') {
                    const itemPath = path.join(this.config.brainPublicContent, item.name);
            ...
            console.log(chalk.blue(`Published files: ${this.processedFiles.size}`));
        } catch (error) {
